...
const translations={
  en:{
    title:"True Gold Calculator",language:"Language",buildings:"Buildings",inventory:"Inventory",
    bonuses:"Bonuses",results:"Results",temperedGold:"Tempered True Gold",trueGold:"True Gold",
    speedups:"Total Speedups (Days)",construction:"Construction Speed %",wolf:"Wolf Skill",
    position:"Position Bonus %",saul:"Saul Skill",king:"King Bonus %",
    preparation:"Kingdom Preparation %",doubleTime:"Double Time (20%)",required:"Required",
    available:"Available",remaining:"Remaining",timeSummary:"Time Summary",
    originalTime:"Original Time",afterBonuses:"After Bonuses",remainingTime:"Remaining Time",
    resources:"Resources",resourcesAfterSaul:"Resources After Saul",food:"Food",wood:"Wood",
    stone:"Stone",iron:"Iron",power:"Power",powerGained:"Total Power Gained",
    copy:"Copy Summary",reset:"Reset",current:"Current Level",target:"Target Level",
    copied:"Copied",none:"None selected"
  },
  ar:{
    title:"حاسبة الذهب",language:"اللغة",buildings:"المباني",inventory:"المخزون",
    bonuses:"المكافآت",results:"النتائج",temperedGold:"الذهب المعدل",
    trueGold:"الذهب الخالص",speedups:"إجمالي التسريعات (بالأيام)",
    construction:"مكافأة سرعة البناء %",wolf:"مهارة الذئب",
    position:"مكافأة المنصب %",saul:"مهارة شاول",king:"مكافأة الملك %",
    preparation:"مكافأة الحدث التحضيري للمملكة ضد المملكة %",
    doubleTime:"الوقت المزدوج (20%)",required:"المطلوب",available:"الموجود",
    remaining:"المتبقي",timeSummary:"ملخص الوقت",originalTime:"الوقت الأصلي",
    afterBonuses:"الوقت بعد المكافآت",remainingTime:"الوقت المتبقي",
    resources:"الموارد",resourcesAfterSaul:"الموارد بعد شاول",food:"الطعام",
    wood:"الخشب",stone:"الحجر",iron:"الحديد",power:"القوة",
    powerGained:"إجمالي القوة المكتسبة",copy:"نسخ الملخص",reset:"إعادة تعيين",
    current:"المستوى الحالي",target:"المستوى الهدف",copied:"تم النسخ",
    none:"لم يتم اختيار مبانٍ"
  }
};

const buildingLabels={
  en:{town:"Town Center",embassy:"Embassy",barracks:"Barracks",stable:"Stable",range:"Range",command:"Command Center",infirmary:"Infirmary",academy:"War Academy"},
  ar:{town:"مركز المدينة",embassy:"السفارة",barracks:"الثكنات",stable:"الإسطبل",range:"معسكر الرماة",command:"مركز القيادة",infirmary:"المستشفى",academy:"أكاديمية الحرب"}
};
...
buildingNames.forEach(name=>{
  ...
  function refreshTargetOptions(){
    ...
    if(target.value && availableLevels.indexOf(target.value)<=currentIndex) target.value="";
  }
  ...
  check.addEventListener("change",()=>{
    ...
    if(!check.checked){current.value="";target.value=""}
    refreshTargetOptions();
    calculate();
  });

  current.addEventListener("change",()=>{refreshTargetOptions();calculate()});
...
const formatTime=seconds=>{
  ...
  return currentLanguage==="ar"?`${days}ي ${hours}س ${minutes}د`:`${days}d ${hours}h ${minutes}m`;
};
...
function calculate(){
  ...
  buildingNames.forEach(name=>{
    ...
    const available=levels.filter(level=>level==="Base"?Boolean(BUILDING_DATA[name]["30-1"]):Boolean(BUILDING_DATA[name][level]));
    ...
    for(let index=start+1;index<=end;index++){
      ...
      const row=level==="Base"?null:BUILDING_DATA[name][level];
      if(!row) continue;
      totals.food+=row.food;totals.wood+=row.wood;totals.stone+=row.stone;totals.iron+=row.iron;
      totals.trueGold+=row.trueGold;totals.temperedGold+=row.temperedGold;totals.time+=row.time;
    }
  ...
  const constructionSpeed=numberValue("constructionBonus")+numberValue("wolfSkill")+numberValue("positionBonus")+numberValue("kingBonus")+numberValue("prepBonus");
  ...
  if(document.getElementById("doubleTime").checked) reducedTime*=0.8;
...
document.getElementById("resetCalculator").addEventListener("click",()=>{
  ...
  document.querySelectorAll("select").forEach(element=>{
    ...
    element.value="";
    if(element.id.endsWith("Current")||element.id.endsWith("Target")) element.disabled=true;
...
document.getElementById("copySummary").addEventListener("click",async()=>{
  ...
  buildingNames.forEach(name=>{
    ...
    const current=document.getElementById(`${name}Current`).value;
    const target=document.getElementById(`${name}Target`).value;
    if(current&&target) selected.push(`${buildingLabels[currentLanguage][name]}: ${current} → ${target}`);
  });
...
الذهب المعدل المطلوب: ${document.getElementById("requiredTemperedGold").textContent}
الذهب المعدل المتبقي: ${document.getElementById("remainingTemperedGold").textContent}
