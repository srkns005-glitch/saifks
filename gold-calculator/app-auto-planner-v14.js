...
const translations={
  en:{
    ...
    target:"Target Level",
    copied:"Copied",
    none:"None selected",plannerTitle:"Auto Upgrade Planner",plannerHelp:"Choose your current and target Town Center levels.",plannerCurrent:"Current Town Center",plannerTarget:"Target Town Center",generatePlan:"Generate Plan",clearPlan:"Clear Plan",planReady:"Upgrade plan generated.",invalidPlan:"Select the current level, then choose a higher target level.",plannerGuideTitle:"How to use",plannerGuide1:"Current level starts at Base. Change it if your Town Center is higher.",plannerGuide2:"Select the level you want to reach.",plannerGuide3:"Press Generate Plan to fill the required buildings automatically.",resetConfirmTitle:"Reset saved data?",resetConfirmText:"Are you sure you want to delete all saved calculator data?",yes:"Yes",cancel:"Cancel"
  },
  ar:{
    title:"حاسبة الذهب",
    buildings:"المباني",
    ...
    copied:"تم النسخ",
    none:"لم يتم اختيار مبانٍ",plannerTitle:"مخطط الترقية التلقائي",plannerHelp:"اختر مستوى مركز المدينة الحالي والمستوى المستهدف.",plannerCurrent:"مستوى مركز المدينة الحالي",plannerTarget:"مستوى مركز المدينة المستهدف",generatePlan:"إنشاء الخطة",clearPlan:"مسح الخطة",planReady:"تم إنشاء خطة الترقية.",invalidPlan:"اختر المستوى الحالي أولًا، ثم اختر مستوى مستهدفًا أعلى منه.",plannerGuideTitle:"طريقة الاستخدام",plannerGuide1:"يبدأ المستوى الحالي من Base، ويمكنك تغييره إذا كان مركز مدينتك أعلى.",plannerGuide2:"اختر المستوى الذي تريد الوصول إليه.",plannerGuide3:"اضغط إنشاء الخطة ليتم تعبئة المباني المطلوبة تلقائيًا.",resetConfirmTitle:"حذف البيانات المحفوظة؟",resetConfirmText:"هل أنت متأكد من حذف جميع بيانات الحاسبة المحفوظة؟",yes:"نعم",cancel:"إلغاء"
  }
};
...
function generateUpgradePlan(){
  ...
  if(!currentTown||!targetTown||plannerLevels.indexOf(targetTown)<=plannerLevels.indexOf(currentTown)){
...
function clearUpgradePlan(){
  ...
  document.getElementById("plannerCurrent").value="Base";
  document.getElementById("plannerTarget").value="";
  document.getElementById("plannerCurrent").disabled=false;
  document.getElementById("plannerTarget").disabled=false;
...
function refreshPlannerTargetOptions(){
  ...
  Array.from(target.options).forEach(option=>{
    ...
    const optionIndex=plannerLevels.indexOf(option.value);
    const shouldHide=currentIndex>=0 && optionIndex<=currentIndex;
  ...
  if(
    target.value &&
    currentIndex>=0 &&
    plannerLevels.indexOf(target.value)<=currentIndex
  ){
    target.value="";
  }
}
...
function restoreState(){
  ...
  try{
    ...
    if(state.planner){
      ...
      if(plannerTarget) plannerTarget.value=state.planner.target||"";
    }
