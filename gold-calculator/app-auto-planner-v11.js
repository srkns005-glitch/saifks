...
const translations={
  ...
  ar:{
    ...
    copied:"تم النسخ",
    none:"لم يتم اختيار مبانٍ",plannerTitle:"مخطط الترقية التلقائي",plannerHelp:"اختر مستوى مركز المدينة الحالي والمستوى المستهدف.",plannerCurrent:"مستوى مركز المدينة الحالي",plannerTarget:"مستوى مركز المدينة المستهدف",generatePlan:"إنشاء الخطة",clearPlan:"مسح الخطة",planReady:"تم إنشاء خطة الترقية.",invalidPlan:"اختر مستوى مستهدفًا أعلى من المستوى الحالي.",resetConfirmTitle:"حذف البيانات المحفوظة؟",resetConfirmText:"هل أنت متأكد من حذف جميع بيانات الحاسبة المحفوظة؟",yes:"نعم",cancel:"إلغاء"
  }
};
...
function clearUpgradePlan(){
  ...
  document.getElementById("plannerTarget").value="TG1";
...
function setupPlanner(){
  ...
  current.addEventListener("change",()=>{
    Array.from(target.options).forEach(option=>{
      option.disabled=plannerLevels.indexOf(option.value)<=plannerLevels.indexOf(current.value);
    });

    if(plannerLevels.indexOf(target.value)<=plannerLevels.indexOf(current.value)){
      target.value=plannerLevels[Math.min(plannerLevels.length-1,plannerLevels.indexOf(current.value)+1)];
    }
...
function collectState(){
  const state={
    ...
    fields:{},
    buildings:{},planner:{current:document.getElementById("plannerCurrent")?.value||"Base",target:document.getElementById("plannerTarget")?.value||"TG1"}
  };
...
function restoreState(){
  ...
  try{
    ...
    if(state.planner){
      ...
      if(plannerTarget) plannerTarget.value=state.planner.target||"TG1";
    }
