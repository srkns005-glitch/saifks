...
function setupPlanner(){
  ...
  current.addEventListener("change",()=>{
    ...
    Array.from(target.options).forEach(option=>{
      ...
      const shouldHide=optionIndex<=currentIndex;
      option.hidden=shouldHide;
      option.disabled=shouldHide;
    });

    if(plannerLevels.indexOf(target.value)<=currentIndex){
      const nextLevel=plannerLevels[currentIndex+1]||plannerLevels[plannerLevels.length-1];
      target.value=nextLevel;
    }
