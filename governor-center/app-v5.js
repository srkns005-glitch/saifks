...
function summaryText(key){
  const terms={
    ...
    ar:{total:"العدد الكلي المطلوب",owned:"الموجود لديك",remaining:"العدد المتبقي"},
    tr:{total:"Toplam Gerekli",owned:"Sahip Olunan",remaining:"Kalan"},
    fr:{total:"Total requis",owned:"Possédé",remaining:"Restant"},
    es:{total:"Total necesario",owned:"Disponible",remaining:"Restante"},
    de:{total:"Insgesamt benötigt",owned:"Vorhanden",remaining:"Verbleibend"},
    ko:{total:"총 필요량",owned:"보유량",remaining:"남은 수량"},
...
function copyGear(){
  ...
  gearDB.slots.forEach(slot=>{
    ...
    lines.push(`${nameOf(slot.id,slot.name)}: ${s.current<0?tr("disabled"):gearDB.levels[s.current].display_name} → ${gearDB.levels[s.target].display_name}`);
  });
  ...
  const gearMaterials=[
    [tr("satin"),req.satin,rem.satin,state.gearOwned.satin],
    [tr("threads"),req.threads,rem.threads,state.gearOwned.threads],
    [tr("visionMaterial"),req.vision,rem.vision,state.gearOwned.vision]
  ];
  gearMaterials.forEach(([label,total,left,owned])=>{
    if(!total)return;
    lines.push(`${label}`);
    lines.push(`${summaryText("total")}: ${fmt(total)}`);
    if(Number(owned)>0){
      lines.push(`${summaryText("owned")}: ${fmt(owned)}`);
      lines.push(`${summaryText("remaining")}: ${fmt(left)}`);
    }
  });
...
function copyCharms(){
  ...
  gearDB.slots.forEach(slot=>{
    charmDB.types.forEach(type=>{
      ...
      lines.push(`${nameOf(slot.id,slot.name)} - ${nameOf(type.id,type.name)}: ${s.current} → ${s.target}`);
    });
  });
  ...
  const charmMaterials=[
    [tr("guides"),req.guides,rem.guides,state.charmOwned.guides],
    [tr("designs"),req.designs,rem.designs,state.charmOwned.designs]
  ];
  charmMaterials.forEach(([label,total,left,owned])=>{
    if(!total)return;
    lines.push(`${label}`);
    lines.push(`${summaryText("total")}: ${fmt(total)}`);
    if(Number(owned)>0){
